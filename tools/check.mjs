...
for (const [id, secs] of RUNS) {
  ...
  await page.evaluate(([x, target]) => {
    ...
    // then PAUSED, so the state the screenshot and the readout sample is the
    // one the driven clock reached and not wherever the frame loop got to in
    // the seconds the capture takes: the same scenario read 17, 22 and 33 per
    // cent damage on one unchanged library before this, sampled at a
    // different simulated time each run
    s.speedIdx = 0;
  }, [id, secs * 700]);
