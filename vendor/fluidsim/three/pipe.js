...
import { liquidMaterial, steamMaterial, casingMaterial, fleckMaterial } from './materials.js?v=d7d0f53269';
import { Tracers, frameOf } from './particles.js?v=d7d0f53269';
...
// The least a vertex must move before the surface counts as having changed.
// A micrometre: below anything a viewer could see and far above the noise of a
// settled solve, which writes the same number twice.
const RIPPLE_EPS = 1e-6;
...
export function ripple(mesh, surface, radius, amp = 1) {
  ...
  // ONLY WHEN SOMETHING MOVED, which is the same rule the colours follow and
  // is exact rather than an approximation: a still surface writes the same
  // heights it already has, and its normals are the ones it already carries.
  //
  // What it saves is not the writes, it is computeVertexNormals: that walks
  // every face, reads three vertices out of the buffer for each, and takes a
  // cross product, and on the consumer's station it was 40 per cent of the
  // whole frame update, 1063 vertices over six surfaces, sixty times a second,
  // on water that was standing still. RIPPLE_EPS is a micrometre; the solve is
  // in metres and a surface that has genuinely settled writes bit-identical
  // heights, so the test is really "has anything happened at all".
  let moved = false;
  const arr = pos.array, st = pos.itemSize;
  for (let i = 0, k = 0; i < pos.count; i++, k += st) {
    // A CircleGeometry lies in xy before the mesh is turned, so the plane the
    // vertices live in is x, y and the height goes on z.
    const z = -surface.sampleAt(arr[k], arr[k + 1], radius, amp);
    const d = z - arr[k + 2];
    if (d > RIPPLE_EPS || d < -RIPPLE_EPS) moved = true;
    arr[k + 2] = z;
  }
  if (!moved) return;
