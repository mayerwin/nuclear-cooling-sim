...
export function frameOf(path, n = 220) {
  ...
  // AND THE ROTATION THAT LIES A SPECK ALONG EACH SEGMENT, which is a property
  // of the PATH and not of the speck: every speck in a segment gets the same
  // one, and the path does not move between frames. It was being rebuilt per
  // speck per frame -- a subtract, a normalise and a setFromUnitVectors, 658
  // times a frame on the consumer's station, sixty times a second, for 220
  // distinct answers that never change. Built here once instead, from
  // pts[j+1] - pts[j], which is exactly the tangent the old line took, so the
  // matrices come out bit for bit the same.
  const qt = [];
  const tv = new THREE.Vector3();
  for (let i = 0; i + 1 < pts.length; i++) {
    tv.copy(pts[i + 1]).sub(pts[i]).normalize();
    qt.push(new THREE.Quaternion().setFromUnitVectors(up, tv));
  }
  return { pts, nrm, bnm, qt };
}
...
export class Tracers {
  ...
  _write(du, v, scale, clipY, dt) {
    // A SET NOBODY CAN SEE DOES NOT GET WRITTEN. `enabled` is set from the
    // run's own state every frame and decides `visible` at the bottom of this
    // method, so a dry line was having all of its matrices recomputed and then
    // hidden. Worse than the arithmetic: the last line of this method sets
    // instanceMatrix.needsUpdate, which makes the renderer re-upload the whole
    // instance buffer, so an invisible set cost a matrix write per speck AND a
    // buffer upload every frame. On the consumer's station that is 19 sets and
    // 658 specks, sixty times a second, for specks nobody is looking at.
    //
    // It stays hidden while it is off, and the first advance after it comes
    // back writes every matrix before it shows, which is the invariant the
    // note at the bottom of this method is about.
    if (this.enabled === false) { this.mesh.visible = false; return; }
    ...
    for (let i = 0; i < this.n; i++) {
      ...
      // The segment's rotation, built once with the frame. See frameOf.
      const q = f.qt ? f.qt[j] : this._q.setFromUnitVectors(_up, _tan.copy(f.pts[j + 1]).sub(f.pts[j]).normalize());
