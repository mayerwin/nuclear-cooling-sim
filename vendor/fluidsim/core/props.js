...
export function psat(T) {
  ...
  // A FOURTH POWER IS TWO MULTIPLIES. Math.pow(d, 4) measured 41 ns of psat's
  // 59, because pow is a general routine that takes logarithms whatever the
  // exponent looks like; d2 * d2 is 2 ns and is the same number to a bit, the
  // exponent being a small power of two.
  const d2 = d * d;
  return d2 * d2 * 1e6;
}
...
for (let i = 0; i <= RHO_N; i++) {
  RHO_L_TAB[i] = rhoLiquidSatExact(T_CRIT * (1 - Math.pow(RHO_UMAX * i / RHO_N, 3)));
}

// THE VAPOUR TABLE IS FOUR TIMES AS LONG AND HOLDS THE DENSITY ITSELF, where
// it used to hold the logarithm and be exponentiated on every read.
//
// The log was there because the vapour density spans four decades and a linear
// interpolation of the value would be badly wrong at the cold end where it is
// smallest. That reasoning is right at 1024 points and stops being right at
// 4096: measured against the published equation over the whole line, the log
// table at 1024 is worst by 1.05e-2 per cent and the direct table at 4096 by
// 3.79e-3, so the table that needs no exponential is also the more accurate of
// the two. It costs 32 KB, and it takes a Math.exp off the path that every
// two-phase cell in a plant walks.
const RHO_V_N = 4096;
const RHO_V_TAB = new Float64Array(RHO_V_N + 1);
for (let i = 0; i <= RHO_V_N; i++) {
  RHO_V_TAB[i] = rhoVapourSatExact(T_CRIT * (1 - Math.pow(RHO_UMAX * i / RHO_V_N, 3)));
}

// ONE CUBE ROOT FOR THREE TABLES.
//
// The two saturated densities and the latent heat are all tabulated against
// u = (1 - T/Tc)^(1/3), for the reason set out above, and every one of them was
// working that root out for itself. A cell in the two-phase region asks for all
// three at the SAME temperature, so it paid for the root three times: measured,
// a cube root is 12.5 ns against the 19 to 29 ns each of those functions costs
// in total, so two thirds of two of them was arithmetic already done.
//
// So the root is taken once and handed to whichever tables want it. The public
// functions still take a temperature and are unchanged to the bit; they are
// simply written in terms of the pair. See stateOf, which is where it pays.
export function satU(T) {
  const t = clamp(T, T_TRIPLE, T_CRIT);
  const tau = 1 - t / T_CRIT;
  // u = 0 is the critical point, and the head of every table IS the critical
  // value, so this needs no branch of its own: the lookups land on entry zero.
  return tau > 0 ? Math.cbrt(tau) : 0;
}

export function rhoLiquidSatAtU(u) {
  const x = u / RHO_UMAX * RHO_N;
  const i = x >= RHO_N ? RHO_N - 1 : (x | 0);
  const f = x - i;
  return RHO_L_TAB[i] + (RHO_L_TAB[i + 1] - RHO_L_TAB[i]) * f;
}

export function rhoVapourSatAtU(u) {
  const x = u / RHO_UMAX * RHO_V_N;
  const i = x >= RHO_V_N ? RHO_V_N - 1 : (x | 0);
  const f = x - i;
  return RHO_V_TAB[i] + (RHO_V_TAB[i + 1] - RHO_V_TAB[i]) * f;
}

// THE ONE-ARGUMENT FORMS KEEP THEIR OWN ROOT rather than calling satU and then
// the table. Writing them as the composition read better and measured worse:
// the worked loop, which is liquid everywhere and asks for one property at a
// time, went five per cent slower on the extra call. A caller that wants
// several properties at one temperature uses satU and the AtU forms; a caller
// that wants one uses these, and neither pays for the other.
...
export function rhoVapourSat(T) {
  ...
  const tau = 1 - t / T_CRIT;
  if (tau <= 0) return RHO_CRIT;
  const x = Math.cbrt(tau) / RHO_UMAX * RHO_V_N;
  const i = x >= RHO_V_N ? RHO_V_N - 1 : (x | 0);
  const f = x - i;
  return RHO_V_TAB[i] + (RHO_V_TAB[i + 1] - RHO_V_TAB[i]) * f;
}
...
export function hfgAtU(u) {
  const x = u / HFG_UMAX * HFG_N;
