...
import { clamp, num, lerp, Table } from './util.js?v=d7d0f53269';
...
//   {kind:'cylinder', r, len, y0, axis}   a drum lying on its side
...
// The radius a [[r, y], ...] profile has at an elevation, linearly between the
// rows that straddle it and flat outside them. Used to read an annulus's inner
// wall at the outer wall's own elevations, so the two need not share rows.
export function radiusOnProfile(profile, y) {
  const n = profile ? profile.length : 0;
  if (n === 0) return 0;
  let prev = profile[0] || [];
  if (y <= num(prev[1], 0)) return Math.max(0, num(prev[0], 0));
  for (let i = 1; i < n; i++) {
    const row = profile[i] || [];
    const y0 = num(prev[1], 0), y1 = num(row[1], 0);
    const r0 = Math.max(0, num(prev[0], 0)), r1 = Math.max(0, num(row[0], 0));
    if (y <= y1) {
      if (!(y1 > y0)) return r1;
      const f = (y - y0) / (y1 - y0);
      return r0 + (r1 - r0) * f;
  ...
  return Math.max(0, num(prev[0], 0));
}
...
export function buildShape(spec, yNode) {
  ...
  // A DRUM ON ITS SIDE, WHICH NO LATHE CAN BE. A lathe turns about the
  // vertical, so it can say tank, vessel, dome and cone and cannot say the one
  // shape half a power station is made of: a condenser shell, a feedwater
  // heater, a receiver, a separator, a road tanker. Written as an area table
  // by hand, as the consumer's condenser was in thirteen rows, it is a curve
  // sampled coarsely where it changes fastest, and it is still not a form
  // anything can draw.
  //
  // Stated as a cylinder it is exact and it is drawable. The free surface at
  // an elevation is the chord of the circle times the length, and the water
  // below an elevation is the circular segment times the length, both in
  // closed form. That matters more here than anywhere else, because the LEVEL
  // in a drum on its side is the least intuitive in a plant: the same tonne of
  // water moves the level fast near the ends and slowly through the middle,
  // and a trapezoid rule over a table gets the ends wrong precisely where the
  // curve is vertical.
  if (kind === 'cylinder') {
    const r = num(s.r, 0), len = num(s.len, 0);
    if (!(r > 0)) throw shapeError('E_SHAPE_SIZE', id, 'a cylinder needs r > 0, got ' + s.r,
      'r is the drum radius in metres; it lies on its side, so 2r is its height');
    if (!(len > 0)) throw shapeError('E_SHAPE_SIZE', id, 'a cylinder needs len > 0, got ' + s.len,
      'len is the drum length along its own axis, in metres');
    const yb = num(s.y0, num(yNode, 0)), yt = yb + 2 * r, yc = yb + r;
    const N = SHAPE_SAMPLES;
    const ys = new Float64Array(N), as = new Float64Array(N), cv = new Float64Array(N);
    for (let i = 0; i < N; i++) {
      // SAMPLED ROUND THE CIRCLE, NOT UP THE DRUM. The chord is vertical at the
      // two ends, where a table read at even heights is at its worst: sixty
      // four even samples put the free-surface area 6 per cent out down there,
      // and the same sixty four spaced by angle put it under a tenth of one.
      // It costs a cosine at build time and nothing at all afterwards.
      const t = Math.PI * i / (N - 1);
      const y = i === N - 1 ? yt : (i === 0 ? yb : yc - r * Math.cos(t));
      ys[i] = y;
      const dyc = y - yc;
      const half = Math.sqrt(Math.max(0, r * r - dyc * dyc));
      as[i] = 2 * half * len;
      // The circular segment below y, exactly: r^2 acos((r-h)/r) minus the
      // triangle, times the length. acos is given a clamped argument because
      // the two ends are where the arithmetic lands just outside [-1, 1].
      const h = Math.min(2 * r, Math.max(0, y - yb));
      const u = Math.min(1, Math.max(-1, (r - h) / r));
      cv[i] = len * (r * r * Math.acos(u) - (r - h) * Math.sqrt(Math.max(0, 2 * r * h - h * h)));
    }
    const Vtot = Math.PI * r * r * len;
    cv[N - 1] = Vtot;
    for (let i = 1; i < N; i++) if (!(cv[i] > cv[i - 1])) cv[i] = cv[i - 1];
    return Object.freeze({
      kind, y0: yb, y1: yt, Vtotal: Vtot,
      cum: new Table(ys, cv), areaT: new Table(ys, as)
    });
  }
  ...
  if (kind === 'box') {
    ...
    // AN ANNULUS IS AN AREA SHAPE, and saying so is the whole implementation.
    // The space between two surfaces of revolution holds pi (ro^2 - ri^2) at
    // every elevation, so once that is worked out there is nothing about it
    // that a jacket, a downcomer, a guide tube or the gap between two shells
    // needs beyond what an area table already does: one integration, one
    // lookup, and the same level and free-surface maths as everything else.
    //
    // The two profiles are read at the OUTER one's elevations, with the inner
    // interpolated onto them, so the pair do not have to be drawn with the
    // same rows. Where the inner is wider than the outer the space is empty
    // rather than negative, which is what the clamp at zero below says.
    const outer = s.outer, inner = s.inner;
    m = outer ? outer.length : 0;
    if (m < 2) throw shapeError('E_SHAPE_Y', id, 'an annulus needs an outer profile of at least two rows',
      'outer and inner are both [[r, y], ...] read bottom upwards');
    if (!inner || inner.length < 2) throw shapeError('E_SHAPE_Y', id,
      'an annulus needs an inner profile of at least two rows',
      'outer and inner are both [[r, y], ...] read bottom upwards');
    ...
    for (let i = 0; i < m; i++) {
      ...
      const y = num(row[1], 0);
      yk[i] = y;
      const ro = Math.max(0, num(row[0], 0));
      const ri = Math.max(0, radiusOnProfile(inner, y));
      const a = Math.PI * (ro * ro - ri * ri);
      vk[i] = a > 0 ? a : 0;
      if (i > 0 && !(yk[i] >= yk[i - 1])) {
        throw shapeError('E_SHAPE_Y', id,
          'y must not decrease, row ' + i + ' is ' + yk[i] + ' after ' + yk[i - 1],
          'a profile is read bottom upwards, in absolute metres');
      }
    }
    ...
    throw shapeError('E_BAD_KIND', id, "unknown shape kind '" + kind + "'",
      "one of 'lathe', 'box', 'area', 'annulus', 'cylinder', 'point'");
  }
