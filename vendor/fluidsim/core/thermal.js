...
import { clamp, lerp, num, growF64, growI32, growU8 } from './util.js?v=d7d0f53269';
import {
  G, psat, tsat, hLiquid, hVapour, hfg, tOfHLiquid, rhoLiquidSat, rhoVapourSat, rhoVapour,
  satU, hfgAtU, rhoLiquidSatAtU, rhoVapourSatAtU,
  cpLiquid, cpVapour, muLiquid, muVapour,
  LIQUID, VAPOUR, TWO_PHASE, T_TRIPLE, T_CRIT, P_TRIPLE, P_CRIT
} from './props.js?v=d7d0f53269';
...
const GAS_BAND = 0.05;
// How far apart the saturation temperatures at an edge's two ends may be
// before the line is evaluated at each cell's own pressure instead of being
// interpolated along the pipe. Eight kelvin, measured: with the pressure
// falling linearly along the edge, which is what a long line with a steady
// loss does, the worst interpolation error is 0.035 K across a 5 K span,
// 0.151 across 10, 2.84 across 43 and 106 across the 246 K a turbine exhaust
// spans between 70 bar and a condenser. Eight keeps it under a tenth.
const SAT_SPAN = 8;
// The most a sub-step may raise a heated edge's own water by, and the specific
// heat it is reckoned with. Five kelvin and 4180 J/kg/K: the temperature is a
// bound on a STEP COUNT rather than a physical quantity, so a reference cp is
// enough and costs nothing, and five is where a released loop starts the right
// way round at every step size measured.
const DT_SUB = 5, CP_SUB = 4180;
// The most inner steps one edge may take inside a sub-step. A cap, not a
// target: it bounds what a single mis-authored edge can cost the frame, the
// way maxSub bounds the network. Sixty-four covers a station's turbine at a
// two second step with room over.
const FINE_MAX = 64;
// WHERE A VESSEL'S STEAM IS, decided by where its heat is going. This is the
// one thing an equilibrium model cannot know from its own state, and it is the
// difference between a condensate pump and a steam trap.
//
// Heat going INTO the water makes vapour IN the water: a pot at a rolling boil
// is bubbles all the way down, and a riser off a submerged nozzle must carry
// the mixture or a natural-circulation loop never starts. Heat coming OUT
// leaves the vapour ABOVE the water, which is a condenser: a suction two metres
// under the surface draws saturated liquid, and drawing the vessel's average
// instead hands the pump something half steam.
//
// Two rates were tried before this and neither can separate the cases. Against
// the vessel's own mass, the thousandth that keeps a settling vessel still also
// stops a pot's riser carrying steam. Against the flow through the nozzle, a
// natural-circulation loop carries a hundred times what it boils and reads as
// quiescent. The sign of the heat is not a threshold at all, and it is the
// physics: bubbles come from heat put into the liquid.   // m
...
// p and gas are written only by closedState, and they are declared here with
// the rest so that no object in the frame path ever grows a property: a shape
// change is a new hidden class and a deoptimisation of every function that has
// seen the old one.
const _state = { T: 288.15, x: 0, rho: 998, mu: 1e-3, cp: 4180, phase: LIQUID, p: 101325, gas: 0 };
const _st2 = { T: 288.15, x: 0, rho: 998, mu: 1e-3, cp: 4180, phase: LIQUID, p: 101325, gas: 0 };
const _stSeal = { T: 288.15, x: 0, rho: 998, mu: 1e-3, cp: 4180, phase: LIQUID, p: 101325, gas: 0 };
...
export function stateOf(p, h, out = _state) {
  ...
  const Ts = tsat(P);
  const hf = hLiquid(Ts);
  // The latent heat is deliberately not computed on the liquid path: it costs a
  // cube root and would dominate the whole thermal pass if it were asked for on
  // every cell of every pipe that is nowhere near boiling.
  if (H < hf) return _liquid(H, out);
  // ONE CUBE ROOT, NOT THREE. The latent heat and the two saturated densities
  // are all tabulated against the same u, and a two-phase cell wants all three
  // at the same temperature; asking each of them for it separately worked the
  // root out three times. See satU in props.js.
  const u = satU(Ts);
  const fg = hfgAtU(u);
  if (H > hf + fg) return _vapour(P, H, Ts, hf + fg, out);
  return _twoPhase(H, Ts, hf, fg, 1 / rhoLiquidSatAtU(u), 1 / rhoVapourSatAtU(u), out);
}

// ---------------------------------------------------------------------------
// A CLOSED VOLUME'S OWN PRESSURE
//
// An open gas space is held at whatever it is open to, and that is a number the
// host states. A CLOSED one is not: the steam in it, the water under it and the
// volume they share decide it between them, and the answer is a state that
// moves. Everything a plant does with pressure comes out of this one closure.
//
// Given the mass a vessel holds, the energy in it and the volume it has, there
// is exactly ONE temperature at which the two phases fit:
//
//     M [ (1-x)/rhoL(T) + x/rhoV(T) ] = V,      x = (h - hf(T)) / hfg(T)
//
// Substituting the second into the first leaves one equation in T. Below the
// point where the liquid alone fills the vessel there is a gas space and the
// liquid is subcooled, so T comes from the enthalpy directly; above the point
// where the liquid alone OVERFILLS it the vessel is water-solid and the
// pressure comes from the bulk modulus, which is the same K_BULK the
// hydraulics already use for a volume's compliance.
//
// The non-condensable gas is carried as n R, a constant of the vessel, and adds
// its own partial pressure by Dalton: a tank of cold water under air stays at
// one atmosphere while its steam partial pressure is two kilopascals, which is
// the case that would otherwise collapse to a vacuum.
//
// One temperature for both spaces, which is the equilibrium assumption every
// plant code makes for a first cut. What it does NOT give is a superheated dome
// over subcooled water, so a pressuriser spray and a stratified tank are the
// next step and not this one. Register C18.
// A CLOSED VESSEL IS ALWAYS FULL OF ITSELF, and that is the whole closure.
// There is no gas space to account for separately: the space above the water IS
// the vapour part of the mixture, its steam is part of the mass, and the two
// together occupy exactly the volume the vessel has. So the fluid's density is
// KNOWN, it is M/V, and the only unknown is the pressure that gives a mixture
// of that density at the energy the vessel holds:
//
//     rho(p, h) = M / V
//
// One equation, one unknown, and monotone: a higher pressure condenses some of
// the vapour, and a mixture with less vapour in it is denser. Every vessel a
// plant has is a point on that curve. A pool 80 per cent full of cold water is
// a mixture at 798 kg/m3 whose quality is a millionth; a boiler shell half full
// at 70 bar is one at 313; a condenser is one at 4. Nothing branches.
//
// The non-condensable gas is carried as n R, a constant of the vessel, and adds
// its partial pressure over the volume the vapour occupies. That is what keeps
// a tank of cold water under air at one atmosphere while the steam in it is at
// two kilopascals, and it is why the same object can be a sealed pressuriser
// and a tank open to the room with nothing but different numbers.
//
// One temperature for both phases, which is the equilibrium assumption every
// plant code makes for a first cut. It does not give a superheated dome over
// subcooled water, so a pressuriser spray and a stratified tank are a further
// step and not this one. Register C18.
const P_CEIL_SEAL = 3e8;             // Pa, the same ceiling the hydraulics use
const SEAL_ITERS = 40;               // bisection in log p: 2^-40 of five decades
const SEAL_LO = 1e3, SEAL_HI = 2.2e7;

export function closedState(M, h, V, nc, pGuess, out = _state) {
  const Vv = V > 1e-9 ? V : 1e-9;
  const Mm = M > 1e-9 ? M : 1e-9;
  const H = clamp(h, H_FLOOR, H_CEIL);
  const want = Mm / Vv;

  // Water-solid, which is a vessel with no vapour left in it at any pressure
  // it could reach. The pressure is then not a vapour pressure at all: it is
  // what it takes to squeeze this much water into this much room, which is the
  // same bulk modulus the hydraulics use for a volume's compliance.
  const Tl = tOfHLiquid(H);
  const rl = rhoLiquidSat(Tl);
  if (want >= rl) {
    _liquid(H, out);
    out.rho = want;
    out.gas = 0;
    out.p = clamp(psat(out.T) + K_BULK * (want / rl - 1), P_TRIPLE, P_CEIL_SEAL);
    return out;
  }

  // And bisection on the rest, in the logarithm because the range is five
  // decades wide: a condenser at five kilopascals and a primary at a hundred
  // and fifty bar are the same equation.
  let lo = Math.log(SEAL_LO), hi = Math.log(SEAL_HI);
  for (let k = 0; k < SEAL_ITERS; k++) {
    const mid = 0.5 * (lo + hi);
    stateOf(Math.exp(mid), H, out);
    if (out.rho < want) lo = mid; else hi = mid;
  }
  const ps = Math.exp(0.5 * (lo + hi));
  stateOf(ps, H, out);
  // What the vapour occupies, which is what the non-condensable is squeezed
  // into. A vessel with no vapour in it has nowhere to put a gas, so it keeps
  // whatever pressure the liquid is at.
  const Vvap = out.x > 0 ? Mm * out.x / rhoVapourSat(out.T) : 0;
  out.gas = Vvap;
  out.p = clamp(ps + (Vvap > 1e-9 ? nc * out.T / Vvap : 0), P_TRIPLE, P_CEIL_SEAL);
  return out;
}
...
// THE WATER THAT ACTUALLY CROSSED THE OUTLET, mass-weighted, in J/kg.
//
// A volume at the end of a pipe used to be billed at the pipe's LAST CELL, and
// that is right only while a sub-step carries the water less than one cell. A
// step that carries it further empties a SLUG several cells long over the
// outlet face, and on a profile that is falling towards the outlet, as every
// exchanger's is, the mean of that slug is much warmer than the last cell
// alone. The vessel was billed for water colder than what reached it, and the
// pipe therefore looked as though it had shed more heat than the wall gave it.
//
// Measured on a bank at 3.5 GW: at a Courant number of 0.76 the edge's own
// energy books are 0.25 per cent out, and at 7.5 they are 9.9 per cent out,
// 351 MW that no wall ever took. Billing the slug takes that to 2.6 per cent
// and leaves the answer BIT-IDENTICAL below one, which is the property that
// matters: this changes nothing for a host that was already taking small
// enough steps, and stops punishing one that is not.
//
// c is the Courant number, cells per sub-step. Reading backwards from the
// outlet costs ceil(c) adds on the one edge that is moving that fast.
export function outflowH(cellH, off, N, cFrac, hIn) {
  const n = N | 0;
  if (!(n > 0)) return hIn;
  const fwd = !(cFrac < 0);
  const last = n - 1;
  const c = cFrac >= 0 ? cFrac : -cFrac;
  // At or below one cell the slug IS the last cell, and returning it rather
  // than a weighted mean of itself keeps the common case exact.
  if (!(c > 1)) return cellH[off + (fwd ? last : 0)];
  let want = c > n ? n : c, acc = 0, taken = 0;
  for (let k = 0; k < n && want > 1e-15; k++) {
    const take = want < 1 ? want : 1;
    acc += take * cellH[off + (fwd ? last - k : k)];
    taken += take;
    want -= take;
  }
  // A pipe flushed clean through in one sub-step: the rest of the slug is
  // water that entered and left within the same sub-step.
  if (c > taken) { acc += (c - taken) * hIn; taken = c; }
  return taken > 1e-15 ? acc / taken : cellH[off + (fwd ? last : 0)];
}
...
// ---------------------------------------------------------------------------
// ndPq AND NOT ndP: this is a pressure a STATE is about to be read at, and a
// node the solve left on its pressure guard has no resolved pressure to offer
// one. ndPq is ndP everywhere else, which is everywhere on a network with no
// cavitating pump. See the loop that fills it in Hydraulic._solveOnce.
function pAtNode(sys, i, y) {
  const p = sys.ndPq && sys.ndPq[i] > 0 ? sys.ndPq[i] : sys.ndP[i];
  if (sys.ndKind[i] !== 1) return p;
  const depth = sys.ndLevel[i] - y;
  return p + (depth > 0 ? sys.ndRhoL[i] * G * depth : 0);
}
...
export class Thermal {
  constructor(sys) {
    ...
    this.cpw = new Float64Array(0);       // cp per cell, held between the two passes of _applySide
    ...
    // How many sub-steps each edge asks for ON ITS OWN, and whether it may
    // take them alone. See subcycles and advect.
    this.egSub = new Float64Array(0);
    this.egAlone = new Uint8Array(0);
    this.egLossW = new Float64Array(0);   // a loss device's W, by edge
    this.egFined = new Uint8Array(0);     // its duty was taken inside advect
    this.nSubNow = 1;                     // the network's count for this step
    ...
    this.holdWas = new Float64Array(0);   // what a held edge's ramp was last laid from
    this._sa = sideRec();
    this._sb = sideRec();
  ...
  size() {
    ...
    // The four numbers a held edge's ramp was last laid from: its two end
    // pressures and its two held temperatures. Seeded to NaN, which compares
    // unequal to everything, so the first sub-step after a rebuild always lays
    // the ramp.
    const hw = this.holdWas;
    this.holdWas = growF64(this.holdWas, 4 * nE);
    if (this.holdWas !== hw) this.holdWas.fill(NaN);
    ...
    this.cpw = growF64(this.cpw, maxCells);
  ...
  _resolveHeat(nH) {
    ...
    this.egSub = growF64(this.egSub, sys.nEdges | 0);
    this.egAlone = growU8(this.egAlone, sys.nEdges | 0);
    this.egLossW = growF64(this.egLossW, sys.nEdges | 0);
    this.egFined = growU8(this.egFined, sys.nEdges | 0);
    ...
    // AND A `loss` DEVICE'S EDGE IS A WORKED EDGE. egHeated is what tells the
    // sub-cycle criterion that an edge has energy going into or out of its
    // water along its length, and a turbine takes more out of a line than any
    // heat link in a plant puts in: 2.7 GW out of 1900 kg/s is 1435 kJ/kg,
    // more than half of what 70 bar steam carries. Left unmarked, the edge was
    // flushed in a single sub-step and the split between transport and the
    // work term lost track of which water had had which joules taken out of
    // it: the consumer's exhaust ran its cells down to the enthalpy FLOOR,
    // -100 kJ/kg, and published 273.15 K, which is what that clamp reads as.
    // The energy was not lost, it was taken from the same kilogrammes more
    // than once.
    this._markLossEdges();
    const net = sys.net;
  ...
  // Every edge a `loss` sits in, marked as worked for the sub-cycle criterion.
  // Walked from the network rather than cached, because it runs once per
  // rebuild beside the heat links themselves.
  // WHICH EDGES MAY TAKE THEIR OWN SUB-STEPS. An edge's transport and its own
  // per-edge duty are local: the water it carries comes from one donor node
  // and what leaves it is billed to one other, and neither is touched again
  // until the sub-step ends. Such an edge can be integrated as finely as it
  // likes inside a sub-step without the rest of the network knowing.
  //
  // AN EXCHANGER SIDE CANNOT. Its duty is computed from BOTH bodies at once
  // and applied to both with opposite signs, so stepping one of them finely
  // while the other stands still would bill a duty that never existed. Nor may
  // an edge whose ends the host is holding, which reads back what was written.
  // Everything else is fair game, which on a station is the turbine's exhaust
  // and the lines that merely move fast.
  _markAlone(nE) {
    const sys = this.sys;
    for (let e = 0; e < nE; e++) this.egAlone[e] = 1;
    for (let k = 0; k < (sys.nHeat | 0); k++) {
      if (this.hlKind[k] !== 1) continue;              // only an exchanger pairs
      if (this.hlAEdge[k] && this.hlA[k] >= 0) this.egAlone[this.hlA[k]] = 0;
      if (this.hlBEdge[k] && this.hlB[k] >= 0) this.egAlone[this.hlB[k]] = 0;
    }
    if (sys.egTHoldOn) for (let e = 0; e < nE; e++) if (sys.egTHoldOn[e]) this.egAlone[e] = 0;
  }

  _markLossEdges() {
    const sys = this.sys;
    const devs = (sys.net && sys.net.devices) || sys.devs;
    if (!devs) return;
    for (let k = 0; k < devs.length; k++) {
      const dv = devs[k];
      if (!dv || dv.kind !== 'loss') continue;
      if (!(num(dv.W, 0) > 0)) continue;
      const e = sys.edgeIds ? sys.edgeIds.index(dv.edge) : -1;
      if (e >= 0) { this.egHeated[e] = 1; this.egLossW[e] = num(dv.W, 0); }
    }
  }
  ...
  subcycles(dt) {
    ...
    this.subWanted = 0;
    this.subCapped = 0;
    // WHICH ONE IS ASKING, and for which of the three reasons. The criterion
    // returns one number for the whole network, so a host told only that it
    // wanted 46 cannot tell whether that is every edge or a single turbine.
    this.subWhy = -1;
    this.subAt = -1;
    for (let e = 0; e < (sys.nEdges | 0); e++) this.egSub[e] = 0;
    this._markAlone(sys.nEdges | 0);
    ...
    for (let i = 0; i < nN; i++) {
      ...
      const r = out * t / M;
      if (r > worst) { worst = r; this.subWhy = 0; this.subAt = i; }
    }
    ...
    // what it pays.
    //
    // Unheated edges are not counted, because an edge nothing heats carries
    // its enthalpy through whatever the step size.
    //
    // AND AN EXCHANGER'S EDGE IS NOT EXEMPT EITHER, which was tried, because
    // its duty comes from the inlet temperatures through the effectiveness
    // blend rather than from a profile along the pipe. On a station whose edge
    // heat is all exchangers, dropping them from the count takes eight
    // sub-steps to one and the tube outlet moves by less than a kelvin, which
    // looked like a free halving of the cost. It is not free: on the worked
    // loop the same change takes the count from eight to four and the heater's
    // rise from 7.1 K to 3.4, because sub-steps buy transport accuracy for
    // every edge and not only for the one being heated. The exemption was
    // reverted. A host that knows its own network can lower maxSub itself and
    // measure what it costs; the default protects the network nobody measured.
    //
    // COUNTING THEM ALL WAS TRIED, to see whether it would stop a released
    // natural-circulation loop stalling at a thirty second step, and it
    // changed no number on any network here: the criterion already asks for
    // more sub-steps than the transport needs. The stall is not a transport
    // failure at all, and the measurement that settles it is in register P8b:
    // a loop STARTED from rest at a thirty second step never gets going, while
    // the same loop already circulating is handed thirty second steps and
    // reproduces the ten second answer exactly. It is a startup problem, and
    // no sub-cycle criterion can see the difference between a network at rest
    // that is about to move and one that is staying put.
    for (let e = 0; e < (this.sys.nEdges | 0); e++) {
      ...
      // FOUR SUB-STEPS PER TURNOVER, not one. An edge flushed in a single
      // sub-step hands its whole duty to water that is then replaced, and an
      // exchanger's answer depends on the profile along it: measured against
      // the closed form for eps-NTU, one turnover per sub-step delivers 0.630
      // of the inlet difference at dt 0.05 and 0.910 at dt 2, and four per
      // turnover delivers 0.630 and 0.690, with dt 0.5 through 5 landing on
      // the same number to the digit. The factor is 2 rather than 4 because
      // the count below doubles. See register C29 for the table and for what
      // is still open.
      const r = 2 * m * t / M;
      if (r > this.egSub[e]) this.egSub[e] = r;
      if (r > worst) { worst = r; this.subWhy = 1; this.subAt = e; }
    }
    // AND THE HEAT GOING IN, WHICH IS THE ONE THING THAT SEES A NETWORK AT REST
    // ABOUT TO MOVE. The two terms above are both read off the flows, so at
    // rest they are both zero and the criterion asks for one sub-step at
    // exactly the moment a natural-circulation loop is deciding which way to
    // turn. It decides wrongly, and arbitrarily: released from rest at a fixed
    // step, examples/loop.json settles FORWARDS at dt 10, 12, 16 and 30 to 34
    // and BACKWARDS at 14 and 18 to 28, with the same 7 kg/s either way. The
    // comment that used to stand here said no sub-cycle criterion could tell a
    // network at rest that is about to move from one that is staying put. That
    // is true of any criterion read from the flows, and false of this one: two
    // megawatts into the heater's own water is 95 K in a twenty second step,
    // and nothing about that number needs a flow to be visible.
    //
    // So a sub-step may not put more than DT_SUB kelvin into a heated edge's
    // own contents. Only a STATED duty is counted, because an exchanger's is
    // the answer to the temperatures rather than a number in the document, and
    // asking for it here would cost an effectiveness solve per link per step
    // to refine a step count. At a frame rate it changes nothing: a 60 Hz host
    // puts a hundredth of a kelvin into an edge per step.
    const heatRate = (e, W) => {
      if (e < 0 || !sys.egOpen[e] || !(W > 0)) return;
      const M = this._mass(e);
      if (!(M > 0)) return;
      // Halved like the edge term, because the count below doubles.
      const r = 0.5 * (W * t / (M * CP_SUB)) / DT_SUB;
      if (r > this.egSub[e]) this.egSub[e] = r;
      if (r > worst) { worst = r; this.subWhy = 2; this.subAt = e; }
    };
    const nH = this.hlObj.length;
    for (let k = 0; k < nH; k++) {
      if (this.hlKind[k] !== 0 || !this.hlAEdge[k]) continue;
      const link = this.hlObj[k];
      let W = link ? num(link.W, 0) : 0;
      if (W < 0) W = -W;
      heatRate(this.hlA[k], W);
    }
    // A turbine's work is the largest of these in a plant by a wide margin, so
    // it is counted here too and not only through egHeated: the flow-based
    // terms cannot see it until the line is already moving.
    const devs = (sys.net && sys.net.devices) || sys.devs;
    if (devs) {
      for (let k = 0; k < devs.length; k++) {
        const dv = devs[k];
        ...
        const W = num(dv.W, 0);
        if (!(W > 0)) continue;
        heatRate(sys.edgeIds ? sys.edgeIds.index(dv.edge) : -1, W);
      }
    }
    const n = Math.ceil(2 * worst);
    // WHAT THE PHYSICS ASKED FOR, BEFORE THE HOST'S CAP, and whether the cap
    // bound. A host that sets maxSub for cost has no other way to find out
    // that it bought the frame time with accuracy: measured on the consumer's
    // station, its turbine exhaust leaves at 350.2 K at maxSub 8 and at 414.2
    // K from 128 upward, sixty-four kelvin of steam temperature decided by a
    // number chosen for cost, silently. The criterion wanted 328 on that edge.
    // Reported rather than enforced, because the trade is the host's to make.
    const want = n < 1 ? 1 : n;
    if (want > this.subWanted) this.subWanted = want;
    if (want > maxSub) this.subCapped++;
  ...
  mixNodes() {
    ...
    for (let k = 0; k < nN; k++) {
      ...
      // ndPq, for the reason given at pAtNode: the node's own state comes from
      // the same pressure the cells of the lines meeting it come from, and
      // neither may be derived from the solve's numerical guard.
      const pq = sys.ndPq && sys.ndPq[i] > 0 ? sys.ndPq[i] : sys.ndP[i];
      if (pq === this.memoP[i] && sys.ndH[i] === this.memoH[i]) continue;
      this.memoP[i] = pq;
      this.memoH[i] = sys.ndH[i];
      const st = stateOf(pq, sys.ndH[i], _state);
      sys.ndT[i] = st.T;
      sys.ndX[i] = st.x;
      sys.ndRho[i] = st.rho;
  ...
  advect(dt) {
    ...
    this._wallE = 0;
    this._fluxE = 0;
    for (let e = 0; e < nE; e++) this.egFined[e] = 0;
    this._wallAbs = 0;
    this._fluxAbs = 0;
    let E = 0;
    for (let e = 0; e < nE; e++) {
      ...
      if (sys.egTHoldOn && sys.egTHoldOn[e]) {
        // A held edge's cells are the host's ramp; what leaves it is its
        // downstream end, as it always was.
        sys.egHOut[e] = sys.egMdot[e] >= 0 ? sys.cellH[off + n - 1] : sys.cellH[off];
        ...
        //
        // AND IT IS LAID ONLY WHEN SOMETHING HAS MOVED. Four numbers decide
        // the whole ramp, so if none of them has changed since the last
        // sub-step the cells already hold the answer and re-deriving it is a
        // property call per cell for nothing. It is an exact test, not a
        // tolerance. Measured on a station with its secondary and passive
        // lines held, at the ten second steps a fast-forwarding host uses:
        // holding everything cost MORE than releasing the primary, 0.322 ms a
        // step against 0.228, because eight sub-steps times thirteen held
        // edges times their cells is most of the frame. A host that holds a
        // settled plant pays nothing for it now.
        ...
        const k = 4 * e;
        if (this.holdWas[k] !== pA || this.holdWas[k + 1] !== pB
          || this.holdWas[k + 2] !== T0 || this.holdWas[k + 3] !== T1) {
          this.holdWas[k] = pA; this.holdWas[k + 1] = pB;
          this.holdWas[k + 2] = T0; this.holdWas[k + 3] = T1;
          for (let j = 0; j < n; j++) {
            const u = (j + 0.5) / n;
      ...
      // HOW FINELY THIS EDGE WANTS TO BE STEPPED, over and above the network.
      //
      // The criterion returns ONE number for the whole network, so every edge
      // is stepped as finely as the worst of them needs. Measured on the
      // consumer's station: its turbine exhaust asks for 96 sub-steps and the
      // entire rest of the plant asks for 8, so a cap set to satisfy the
      // turbine costs the other thirty-odd edges twelve times what they need,
      // and a cap set for cost leaves the exhaust sixty-four kelvin wrong.
      //
      // An edge that may go alone takes the difference by itself: its own
      // transport and its own per-edge duty are local, so it can run k inner
      // steps of (carry, then take the work out) inside one sub-step while the
      // network takes one. What leaves it is the mean of what left on each
      // inner step, so the vessel at its end is billed for the same water it
      // would have been billed for had the whole network been stepped that
      // finely. See _markAlone for which edges qualify and why.
      const nSub = this.nSubNow > 0 ? this.nSubNow : 1;
      const want = this.egSub[e] > 0 ? Math.ceil(2 * this.egSub[e]) : 1;
      const fine = (this.egAlone[e] && want > nSub && sys.egOpen[e])
        ? Math.min(FINE_MAX, Math.ceil(want / nSub)) : 1;

      // The SAME mass the duty was spread over, not a second reading of the
      // density taken after the duty moved it. See _mass.
      const mCell = this._mass(e) / n;
      ...
      // A SUBMERGED NOZZLE STILL DRAWS THE BULK, and it was tried the other
      // way round. A condenser hotwell holds its uncondensed steam ABOVE the
      // water, so a suction two metres under the surface should draw saturated
      // liquid and not a mixture that is half vapour; a boiling vessel holds
      // its steam as bubbles THROUGH the water, so a riser off the same kind
      // of nozzle must carry the mixture or a natural-circulation loop never
      // starts. Both are "a free volume with a quality" and this library
      // cannot tell them apart, because it has no gas space of its own: that
      // is register line C18, and drawing liquid here made the boiling riser
      // carry x = 0.005 where it needs steam. The bulk enthalpy is the only
      // answer available until a volume's vapour has somewhere to be.
      // The same rule read from the pipe's end: what ENTERS a line from a
      // vessel is what stands at its nozzle. Above the surface, steam; below
      // it, water, unless the vessel is boiling and the water is full of
      // bubbles. See the outflow in volumes() for why each half is there.
      const fl = sys.egFluid ? sys.egFluid[e] : null;
      let hIn = sys.ndH[donor];
      if (sys.ndFree[donor]) {
        const yIn = sys.egMdot[e] >= 0 ? sys.egYFrom[e] : sys.egYTo[e];
        const steamThere = sys.ndX[donor] > 1e-6 || (fl && fl.vapour);
        if (steamThere && yIn > sys.ndLevel[donor] - GAS_BAND) hIn = hVapour(sys.ndT[donor]);
        else if (sys.ndX[donor] > 0
          && !(sys.ndBubbly && sys.ndBubbly[donor])) hIn = hLiquid(sys.ndT[donor]);
      }
      // BEFORE the profile is overwritten: what crossed the outlet is the slug
      // that was standing at it, and it leaves without this sub-step's wall
      // heat, because transport runs first.
      if (fine <= 1) {
        sys.egHOut[e] = outflowH(sys.cellH, off, n, cFrac, hIn);
        advectEdge(sys.cellH, off, n, cFrac, hIn, this.tmp);
      } else {
        // k inner steps of the same pair, in the same order the sub-step uses:
        // carry the water, then take this slice of the work out of it. The
        // outflow is the mean of the k slugs, each carrying the same mass.
        const cf = cFrac / fine, W = this.egLossW[e];
        let acc = 0;
        for (let q = 0; q < fine; q++) {
          acc += outflowH(sys.cellH, off, n, cf, hIn);
          advectEdge(sys.cellH, off, n, cf, hIn, this.tmp);
          if (W > 0) {
            // One refresh before every duty, as the sub-step itself does: the
            // work condenses the steam as it goes and the mass it is spread
            // over has to follow. See the note in wall().
            this._refresh(e);
            this._lossW(e, W, t / fine);
          }
        }
        sys.egHOut[e] = acc / fine;
        this.egFined[e] = 1;
      }
    }
  ...
  wall(dt) {
    ...
    if (!(t > 0)) return;
    // WORK TAKEN OUT OF THE FLOW, which is the other half of the loss device.
    // Energy that leaves as work leaves the water: a turbine's steam arrives at
    // the condenser colder than it left the boiler, and a library that took the
    // pressure without the enthalpy would send saturated steam to a condenser
    // that had nothing to condense. It is a sink on that edge and nothing more,
    // which is what it is, so it goes through the same path a sink does.
    // ONE REFRESH, AND EVERY DUTY TAKEN AFTER IT. Only the edges a link or a
    // loss actually touches need their cell temperatures refreshed before the
    // duties are worked out; the rest are refreshed once at the end of the
    // sub-step. egHeated covers both since a loss device marks its edge.
    //
    // THE ORDER IS THE WHOLE POINT. This pass used to run AFTER the loss loop,
    // which refreshed its own edge first, so a worked edge was refreshed twice
    // with the duty in between: the duty was spread over the mass the edge had
    // BEFORE the work condensed its steam, and the transport that follows
    // divided by the mass it had AFTER. On the consumer's turbine exhaust,
    // where the density nearly doubles across a sub-step, those were 2584 kg
    // and 4946 kg, a ratio of 1.91, and the steady gradient came out 2747
    // kJ/kg against the 1435 that W/mdot allows. Neither reading was wrong on
    // its own; they were answers to the same question asked at two different
    // moments.
    ...
    if (devs) {
      for (let k = 0; k < devs.length; k++) {
        ...
        const e = sys.edgeIds ? sys.edgeIds.index(dv.edge) : -1;
        if (e < 0) continue;
        // Already taken, one slice at a time, inside advect. See the fine
        // branch there.
        if (this.egFined[e]) continue;
        this._lossW(e, W, t);
      }
    }
    if (nH <= 0) return;
  ...
  // A stated power out of one edge, spread evenly along it. The same
  // arithmetic as a uniform sink, written separately because a loss device is
  // not a heat link and has no profile of its own to ask for.
  _lossW(e, W, dt) {
    const sys = this.sys;
    ...
    for (let j = 0; j < n; j++) {
      ...
      this._wallAbs += Math.abs(mCell * (after - before));
    }
  }
  ...
  _exchanger(k, link, dt) {
    ...
    let Q = Qlump + (Qntu - Qlump) * w;
    if (!Number.isFinite(Q) || Q === 0) return;

    // ONE DUTY, NOT TWO. Each side can only take so much before it lands on
    // the other's temperature, and both sides used to be clamped to their own
    // limit INDEPENDENTLY: a vessel that could give a little gave a little,
    // the stream that was asking for a lot received a lot, and the difference
    // was energy that never existed. Measured on a station: a condenser
    // holding twelve kilograms was billed 8.5 GW by its own sea tubes while
    // receiving 0.6, and the sea water came out 34 K hotter than anything had
    // heated it. A vessel is a reservoir only in the sense that it presents no
    // capacity RATE; it still holds a finite amount of heat, and Cmax being
    // infinite says nothing about that.
    //
    // So the duty is the smaller of what the two sides can take, computed
    // before either is touched, and the same number is applied to both with
    // opposite signs. It does nothing when a vessel is large, which is every
    // pool and every tank; it is the whole answer when it is not.
    const capA = this._capSide(a, -Q, b, dt);
    const capB = this._capSide(b, Q, a, dt);
    const lim = capA < capB ? capA : capB;
    if (!(lim > 0)) return;
    const mag = Q >= 0 ? Q : -Q;
    if (mag > lim) Q = Q >= 0 ? lim : -lim;
  ...
  // The largest duty of this sign a side can absorb before it arrives at the
  // other side's temperature, as a magnitude. Infinity where there is nothing
  // to limit: a boundary is held for ever and is a true reservoir, and a
  // junction holds no heat and takes none, so neither bounds the exchange.
  _capSide(s, Q, other, dt) {
    const sys = this.sys;
    if (s.idx < 0) return Infinity;
    if (!s.isEdge) {
      if (sys.ndKind[s.idx] !== 1) return Infinity;
      // WHAT A VESSEL CAN GIVE IS AN ENTHALPY, AND IT INCLUDES WHAT PASSES
      // THROUGH IT. Both halves of that matter and each was wrong on its own.
      //
      // In enthalpy, because a condenser gives up LATENT heat: cp times a
      // temperature difference is 45 kJ/kg where the answer is 2400, a factor
      // of fifty, and _cpOf is a blend of the two phases' specific heats with
      // no latent term in it at all. The bound is the enthalpy this vessel
      // holds above the one it would hold at the partner's temperature.
      //
      // And including the throughput, because a condenser is mostly a
      // pass-through: what it can condense in a step is the steam ARRIVING in
      // that step as well as the water already standing in it. Bounded by the
      // standing inventory alone, a plant condenser passing 1900 kg/s became a
      // twelve kilogram bucket that condensed nothing, which is how the last
      // fix traded an invented duty for a missing one.
      //
      // The target state is resolved ACROSS THE SATURATION LINE, or the bound
      // is nonsense in exactly the two cases that matter. A vessel being
      // heated by something hotter than its own boiling point can absorb its
      // whole latent heat, so the target is saturated VAPOUR; asking instead
      // for the enthalpy of a liquid at the partner's temperature gets the
      // saturation clamp and a limit of nearly zero, which turned a steam
      // generator's 2197 MW into 1064. A vessel being cooled by something
      // below its boiling point can give up its whole latent heat, so the
      // target is liquid at the partner's temperature.
      const pN = sys.ndP[s.idx];
      const hNow = sys.ndH[s.idx];
      const ts = tsat(pN);
      const hAt = other.Tmean >= ts ? hVapour(ts) : hLiquid(other.Tmean);
      const mass = s.M + this._inflow(s.idx) * dt;
      const qmax = dt > 0 ? mass * (hAt - hNow) / dt : 0;
      // Of the right sign only: a duty pointing away from the partner is not
      // limited by the partner, it is simply not available.
      if (Q > 0) return qmax > 0 ? qmax : 0;
      if (Q < 0) return qmax < 0 ? -qmax : 0;
      return 0;
    }
    ...
    const denom = s.M * s.cp * (sumD / n);
    if (!(s.teff > 0)) return 0;
    const cap = denom / s.teff;
    if (Q > 0) return cap > 0 ? cap : 0;
    if (Q < 0) return cap < 0 ? -cap : 0;
    return 0;
  }

  // The mass arriving at a volume, kg/s, over every open edge that points into
  // it. A vessel's capacity over a step is what stands in it plus this.
  _inflow(i) {
    const sys = this.sys;
    let into = 0;
    const s0 = this.adjStart[i], s1 = this.adjStart[i + 1];
  ...
  _applySide(s, Q, other, dt) {
    ...
    const n = s.n, off = s.off;
    const mCell = s.M / n;
    // THE SUM THE SPLIT IS ACTUALLY GOING TO PAY OUT, not a mean of it. Each
    // cell closes the fraction f of its own gap, converted at ITS OWN cp, so
    // the energy delivered is f times the sum of cp times gap. Setting f from
    // a single mean cp and a single mean gap instead made the two differ by
    // the covariance between them, and cp moves by a sixth across a primary's
    // temperatures: measured on a 3.5 GW bank, 2.5 per cent more energy
    // delivered than the duty asked for, every sub-step, in the same
    // direction. Summing it costs one multiply per cell in a loop that was
    // already walking them.
    //
    // The cp of each cell is KEPT between the two passes rather than looked up
    // twice. A property call is not free and this loop runs per exchanger per
    // sub-step: measured on the consumer's station, computing it twice cost 11
    // per cent of the whole step, and holding it costs nothing that shows.
    const cpw = this.cpw;
    let sumCpD = 0;
    for (let j = 0; j < n; j++) {
      const T = sys.cellT[off + j];
      const cp = _cpOf(T, sys.cellX[off + j]);
      cpw[j] = cp;
      sumCpD += cp * (this._partnerT(s, other, j) - T);
    }
    const f = clamp(Q * s.teff / (mCell * sumCpD), 0, 1);
    if (!(f > 0)) return;
    for (let j = 0; j < n; j++) {
      const T = sys.cellT[off + j];
      const dh = f * cpw[j] * (this._partnerT(s, other, j) - T);
      sys.cellH[off + j] = clamp(sys.cellH[off + j] + dh, H_FLOOR, H_CEIL);
      this._wallE += mCell * dh;
    }
  }
  ...
  // WHAT AN EDGE WEIGHS. Read off its mean density, so it moves as the state
  // does, and every term inside a sub-step has to be reading the SAME state.
  // That is a question of ordering, not of caching: see _applyHeat, where the
  // duties are now all taken after one refresh rather than around two.
  //
  // Settling it in a per-sub-step cache instead was tried and reverted. It
  // makes the pair that must agree agree, but it also freezes the mass an
  // EXCHANGER reckons its lumped capacity from, and that is supposed to follow
  // the state: measured on the consumer's governed station, its steam flow
  // went from 1892 kg/s to 3713 and its exhaust back on to the enthalpy floor.
  _mass(e) {
    const sys = this.sys;
  ...
  volumes(dt) {
    ...
    for (let i = 0; i < nN; i++) {
      ...
      // Hoisted above the flow loop because the draw rule below needs it: what
      // a submerged nozzle gets depends on whether this vessel is being heated
      // or cooled. ndQ is the solver's channel, nodeQ is the exchanger duty
      // this file worked out.
      const heatIn = (sys.ndQ ? sys.ndQ[i] : 0) + this.nodeQ[i];
      const bubbly = heatIn > 0 && sys.ndX[i] > 0;
      if (sys.ndBubbly) sys.ndBubbly[i] = bubbly ? 1 : 0;
      ...
      for (let a = s0; a < s1; a++) {
        ...
        const off = sys.egOff[e], n = sys.egCells[e] | 0;
        if (into > 0) {
          // What arrives carries the enthalpy of the water that CROSSED the
          // outlet over this sub-step, which is the cell touching this node
          // only while the step carries the water less than one cell. See
          // outflowH and C29.
          const h = n > 0 ? sys.egHOut[e] : sys.ndH[i];
          fin += into;
          finH += into * h;
          ...
          // A NOZZLE DRAWS WHAT IT STANDS IN, and where it stands is geometry,
          // not what the line is called. Above the surface is the vapour
          // space, and what leaves there is steam at the vessel's own
          // temperature, carrying about seven times the enthalpy per kilogram
          // that the water under it does: a vessel whose outflow left at its
          // MIXED enthalpy would lose mass without the latent heat that mass
          // took with it, and would boil for ever on a fraction of the energy
          // boiling actually costs.
          //
          // Below the surface is water, and the mixed enthalpy is wrong there
          // too: a condenser hotwell holds its uncondensed steam ABOVE the
          // water, so a suction two metres under should draw saturated liquid
          // and not a mixture that is half vapour. UNLESS THE VESSEL IS
          // BOILING, because then the steam is bubbles distributed THROUGH the
          // water and a riser off a submerged nozzle must carry the mixture or
          // a natural-circulation loop never starts. The boil rate is the
          // discriminator and it is already known; drawing liquid without that
          // test was tried and it made a boiling riser carry x = 0.005 where
          // it needs steam.
          //
          // And there has to BE steam there. A cold tank's gas space holds
          // whatever the host says it holds and this library carries no mass
          // for it, so a nozzle standing in it draws the water below rather
          // than a vapour that is not there: without that test a tank draining
          // through a nozzle at its own water line, and a break discharging
          // from one, both came out carrying steam.
          const free = sys.ndFree[i];
          const above = yEnd2 > sys.ndLevel[i] - GAS_BAND;
          const steamThere = sys.ndX[i] > 1e-6;
          const drawsGas = free && above && (steamThere || (flo && flo.vapour));
          const drawsLiquid = free && !above && sys.ndX[i] > 0 && !bubbly;
          foutH += (-into) * (drawsGas ? hVapour(sys.ndT[i])
            : (drawsLiquid ? hLiquid(sys.ndT[i]) : sys.ndH[i]));
      ...
      // A CLOSED VESSEL DECIDES ITS OWN PRESSURE. Everything it holds and the
      // room it holds it in fix the state between them, and the pressure that
      // comes out is what the hydraulics see at that node on the next
      // sub-step: a boiler that is boiling harder than its steam line takes
      // pushes its own pressure up, a condenser that cannot condense what
      // arrives rises off its vacuum, and a pool under a lid relieves instead
      // of sitting at its own boiling point for ever. An OPEN space is held at
      // what it is open to and reads its state at that pressure, as before.
      const shape0 = sys.ndShape ? sys.ndShape[i] : null;
      const Vtot0 = shape0 && shape0.Vtotal > 0 ? shape0.Vtotal : 0;
      //
      // UNLESS THE HOST IS HOLDING THE PRESSURE, in which case the vessel is
      // not deciding anything: the host is, and the state has to be the one at
      // the pressure the host is going to leave it at. Resolving it here with
      // closedState and letting the hold overwrite the pressure afterwards
      // leaves a temperature and a quality belonging to a DIFFERENT pressure,
      // and every one of them is published. Measured on the consumer's station
      // with its vessels pinned as its app pins them: the reactor vessel read
      // 601.42 K at x 0.037 with its pressure reading 155.00 bar, where
      // saturation is 617.94 K and its own 1575.4 kJ/kg is subcooled water at
      // 607.27. The pair belonged to the 130 bar closedState had chosen.
      //
      // It cannot be repaired in the hold either, and that was tried: the
      // quality would then change after this function has already measured how
      // much mass changed phase, so a vessel that flashed on a falling held
      // pressure reported no boiling at all and the picture lost its bubbles.
      // The state, the pressure and the phase change are one answer and they
      // are computed in one place.
      const closed = sys.ndClosed && sys.ndClosed[i] && Vtot0 > 0
        && !(sys.ndPHold && sys.ndPHold[i]);
      const st = closed
        ? closedState(Mnew, hNew, Vtot0, sys.ndNc[i], sys.ndP[i], _state)
        : stateOf(sys.ndP[i], hNew, _state);
      if (closed) sys.ndP[i] = st.p;
      ...
      // A VESSEL THAT HOLDS MORE THAN IT CAN IS A FACT THE HOST MUST BE TOLD.
      // Nothing here bounds what ARRIVES: the inventory clamp limits what may
      // leave, because that is the side where h = U/M goes infinite. So a
      // volume takes whatever the hydraulics deliver, and when the hydraulics
      // are wrong the volume is where it shows up as a number a person can
      // read. Measured: a 480 cubic metre vessel came to hold 499 tonnes, an
      // implied 1039 kg/m3 of water that is at 618 K and weighs 594. It
      // arrived in ONE ten second step, against a pressure move of a quarter
      // of a bar that its compliance says is worth three kilogrammes.
      //
      // The cause was a Newton that stopped on its iteration budget with the
      // node balances still open, and the volume integrated the open residual
      // as if it were flow: see DEFAULT_OPTS.maxIter for the measurement that
      // fixed it. This report is what named the vessel, so it stays.
      //
      // It is not clamped. Clamping what has already flowed in would break
      // continuity at both ends of the pipe it came down, which is the one
      // thing this library will not do quietly.
      //
      // The WORST excess standing at the end of a sub-step, and the vessel it
      // stands in, rather than a sum over the sub-steps: summing reported 1582
      // tonnes for a vessel that was 197 over, and a host cannot compare that
      // number to anything it knows.
      //
      // AND ONLY WHERE THE INVENTORY AND THE DENSITY ARE TIED, which is to say
      // only for a volume with a free surface. `free: false` is the host
      // saying this vessel is water-solid at a pressure it is asserting, and
      // C18 honours that by letting the state follow the pressure while the
      // inventory follows the flows: the two are decoupled ON PURPOSE. So a
      // primary vessel authored at 310 C and settled at its saturation
      // temperature holds 691 kg/m3 of water the state function now calls 594,
      // and a capacity built from that density calls a perfectly healthy
      // reactor vessel 47 tonnes overfull, every step, for ever. A report a
      // host has to learn to ignore is worse than no report.
      const cap = shape0 && shape0.Vtotal > 0 ? shape0.Vtotal * rhoL : 0;
      if (cap > 0) {
        sys.ndMmax[i] = cap;
        const over = Mnew - cap;
        if (sys.ndFree[i] && over > cap * 1e-6 && over > this._overfill) {
          this._overfill = over;
          this._overfillAt = i;
        }
      }
  ...
  _finish(dt) {
    ...
    if (rep) {
      ...
      rep.overfilled = this._overfill;
      rep.overfilledNode = this._overfillAt;
    }
  }
  ...
  _refresh(e) {
    ...
    //
    // UNLESS THE PREMISE IS FALSE, AND ON ONE EDGE IN A PLANT IT IS. A turbine
    // is a `loss` in a line, and that line spans 70 bar at its inlet to 74
    // millibar at the condenser: the saturation temperature across it runs 559
    // K to 313, and interpolated along the PIPE rather than against the cell's
    // own pressure it runs off the bottom of the scale and lands on the triple
    // point. Measured on such an edge, the far cell published 325.5 K where
    // its own pressure of 3.57 bar puts saturation at 412.7, and the
    // consumer's exhaust published 273.1 K, which is 0 C in a line full of wet
    // steam. The enthalpy was right the whole way; only the temperature read
    // off it was wrong, so nothing downstream of the enthalpy noticed and the
    // picture was the only thing that could have said.
    //
    // So the two-end sampling is kept where its premise holds and the line is
    // evaluated at each cell's own pressure where it does not. The test is the
    // span it would have to interpolate across: below SAT_SPAN the error is
    // far under a tenth of a kelvin, and above it there is no interpolation
    // worth doing.
    ...
    // Wide, so every cell is asked its own pressure. One edge in a station is,
    // and it is the one a turbine sits in.
    const wide = (TsA > TsB ? TsA - TsB : TsB - TsA) > SAT_SPAN;
    let rho = 0, mu = 0, xs = 0, liq = 0;
    for (let j = 0; j < n; j++) {
      const p = sys.cellP[off + j];
      const u = (j + 0.5) / n;
      const Ts = wide ? tsat(clamp(p, P_TRIPLE, P_CRIT)) : TsA + (TsB - TsA) * u;
      const hf = wide ? hLiquid(Ts) : hfA + (hfB - hfA) * u;
      const fg = wide ? hfg(Ts) : fgA + (fgB - fgA) * u;
      const H = clamp(sys.cellH[off + j], H_FLOOR, H_CEIL);
      const vf = wide ? 1 / rhoLiquidSat(Ts) : vfA + (vfB - vfA) * u;
      const st = H < hf ? _liquid(H, _st2)
        : (H > hf + fg ? _vapour(clamp(p, P_TRIPLE, P_CRIT), H, Ts, hf + fg, _st2)
          : _twoPhase(H, Ts, hf, fg, vf,
            wide ? 1 / rhoVapourSat(Ts) : vgA + (vgB - vgA) * u, _st2));
    ...
    sys.egRho[e] = rho / n;
    sys.egMu[e] = mu / n;
    const fill = clamp(liq / n, 0, 1);
    sys.egFill[e] = fill;
    // WHAT KIND OF RUN THIS IS, BY VOLUME AND NOT BY MASS, because volume is
    // what the eye reads and mass is not. A riser carrying half a per cent
    // steam BY MASS at one atmosphere is nine tenths steam BY VOLUME: the
    // water in it is a film on the wall and what you see down the bore is
    // vapour. Keyed on quality it published itself as a liquid run, which is
    // the one thing the picture must never say about a pipe you can see
    // through. egFill is already that fraction, computed cell by cell from the
    // two phase densities a few lines above.
    sys.egKind[e] = fill > 0.99 ? 0 : (fill < 0.01 ? 2 : 1);
  }
