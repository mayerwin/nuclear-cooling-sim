...
import { polylineLength, buildShape, splitPts } from './geometry.js?v=d7d0f53269';
import { hasFluid, fluidNames, fluid, degK } from './props.js?v=d7d0f53269';
...
// `loss` is the generic component: a stated pressure drop, a stated power
// taken out of the flow, or both, on any edge. A turbine, a governor valve
// throttling, a filter, a fitting nobody wanted to compute a K for: this
// library has no business knowing which. PRESSURE AND HEAT ARE THE ONLY THINGS
// A COMPONENT MAY DO TO A FLUID, so those are the only two numbers it takes.
export const DEVICE_KINDS = Object.freeze(['pump', 'valve', 'check', 'orifice', 'break', 'loss']);
export const HEAT_KINDS = Object.freeze(['source', 'sink', 'exchanger', 'ambient']);
export const SHAPE_KINDS = Object.freeze(['lathe', 'box', 'area', 'annulus', 'cylinder', 'point']);
export const NORMALISE_KINDS = Object.freeze(['run', 'network', 'absolute']);

// How far apart in ELEVATION the ends meeting at one junction may be drawn
// before it is worth saying so. Five centimetres: far above the rounding in a
// hand-drawn centreline and far below anything that is a real height.
const JOINT_DY = 0.05;              // m
...
function checkNode(raw, path, errors) {
  ...
  if (raw.gas != null && isObj(raw.gas) && raw.gas.closed != null
      && typeof raw.gas.closed !== 'boolean') {
    push(errors, 'E_TYPE', path + '/gas/closed',
      'node "' + id + '" has gas.closed that is not a boolean',
      'gas: { p, closed } - closed means the space is sealed and its pressure is a state, not a number the host holds');
  }
  if (raw.gas != null) {
...
function checkShape(s, path, id, kind, errors) {
  ...
  if (s.kind === 'cylinder') {
    if (!fin(s.r) || s.r <= 0 || !fin(s.len) || s.len <= 0) {
      return push(errors, 'E_SHAPE_SIZE', path,
        'cylinder shape on "' + id + '" needs r and len both greater than zero',
        'a drum lying on its side: r is its radius, len its length, y0 the elevation of its bottom');
    }
    if (s.axis != null && s.axis !== 'x' && s.axis !== 'z') {
      push(errors, 'E_BAD_KIND', path + '/axis',
        'cylinder shape on "' + id + '" has axis ' + JSON.stringify(s.axis) + '; it must be "x" or "z"',
        'which way the drum lies in plan; it is never vertical, which is what a lathe is for');
    }
    return;
  }
  if (s.kind === 'box') {
    ...
    // Two profiles, each checked exactly as a lathe's is. The space between
    // them is what the volume is, so both have to be there and both have to be
    // drawn bottom upwards; whether one is inside the other at every elevation
    // is not an error, because a jacket that closes at the top is drawn by
    // letting the two meet.
    for (const key of ['outer', 'inner']) {
      const rows = s[key];
      if (!Array.isArray(rows) || rows.length < 2) {
        return push(errors, 'E_SHAPE_SIZE', path + '/' + key,
          'annulus shape on "' + id + '" needs an ' + key + ' profile of at least two entries',
          'outer and inner are both [[r, y], ...] read bottom upwards, in absolute metres');
      }
      let prev = -Infinity;
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!Array.isArray(row) || row.length < 2 || !fin(row[0]) || !fin(row[1])) {
          return push(errors, 'E_SHAPE_SIZE', path + '/' + key + '/' + i,
            'annulus shape on "' + id + '" has a bad entry at ' + i + '; each row is [r, y], two finite numbers');
        }
        if (row[1] < prev) {
          return push(errors, 'E_SHAPE_Y', path + '/' + key + '/' + i,
            'annulus shape on "' + id + '" has y going backwards in ' + key + ' at entry ' + i,
            'the elevations must not decrease, and they are absolute');
        }
        prev = row[1];
      }
    }
...
function checkDevice(raw, path, nodeIds, edgeIds, errors) {
  ...
  if (raw.kind === 'pump') {
    ...
    checkNumbers(raw, path, ['speed', 'inertia', 'cmd', 'npsh'], errors);
  ...
  } else if (raw.kind === 'loss') {
    checkNumbers(raw, path, ['dp', 'W'], errors);
    if (num(raw.dp, 0) < 0) {
      push(errors, 'E_RANGE', path + '/dp',
        'device "' + id + '" has a negative dp',
        'a loss takes pressure OUT of the flow; a device that puts pressure in is a pump');
    }
...
function checkGraph(nodes, edges, devices, heat, opts, errors, warnings) {
  ...
  // THE LINES THAT MEET AT A JUNCTION HAVE TO AGREE ABOUT HOW HIGH IT IS. A
  // junction is a point and holds nothing, but the solver reads the ELEVATION
  // of each line's end off THAT LINE'S OWN centreline, so two pipes drawn to
  // ends half a metre apart are telling it that one point is at two heights.
  // The static head round every loop through that junction is then wrong by
  // the difference, which is nothing on a primary at 155 bar and is everything
  // in a passive loop driven by a metre of thermal head.
  //
  // NOT the distance from the junction's `at`, which was the obvious check and
  // is the wrong one: `at` is where the label goes, and the pipe centrelines
  // carry the real geometry. Measured on this repository's own station, moving
  // every end onto its junction's `at` made the plant WORSE, 26 unconverged
  // steps of 300 to 27 and 3151 kg adrift to 6235, because it moved the ends
  // to the label's height. Moving them instead to the mean of their own ends,
  // so they merely AGREE, took it to 24 and 2346. Both this station and the
  // consumer's carry disagreements of up to 1.1 m and have all along.
  //
  // A warning and not an error, for that reason: documents that do this work,
  // and this library does not refuse a document that works. The consumer met
  // it by moving every end 0.3 m along its own line so the model could cut a
  // nozzle, which also moved the ends sitting on junctions; one tee's two
  // lines ended 0.6 m apart and that unit's health table went from clean to
  // 284 and 618 unconverged steps with 24 to 55 tonnes adrift on every
  // accident row, while every headless fixture stayed clean because those hold
  // the flows the app lets go.
  const jointY = new Map();
  for (let e = 0; e < edges.length; e++) {
    const eg = edges[e];
    const pts = eg && eg.pts;
    if (!Array.isArray(pts) || pts.length < 2) continue;
    for (let end = 0; end < 2; end++) {
      const i = idx.get(end === 0 ? eg.from : eg.to);
      if (i == null || nodes[i].kind !== 'junction') continue;
      const pt = pts[end === 0 ? 0 : pts.length - 1];
      if (!Array.isArray(pt) || pt.length < 3 || !fin(pt[1])) continue;
      let rec = jointY.get(i);
      if (!rec) { rec = { lo: Infinity, hi: -Infinity, loAt: '', hiAt: '' }; jointY.set(i, rec); }
      if (pt[1] < rec.lo) { rec.lo = pt[1]; rec.loAt = eg.id; }
      if (pt[1] > rec.hi) { rec.hi = pt[1]; rec.hiAt = eg.id; }
    }
  }
  for (const [i, rec] of jointY) {
    const d = rec.hi - rec.lo;
    if (!(d > JOINT_DY)) continue;
    push(warnings, 'W_JOINT_SPLIT', '/nodes/' + i,
      'the lines meeting at junction "' + nodes[i].id + '" disagree about its height by '
        + d.toFixed(2) + ' m: "' + rec.loAt + '" ends at ' + rec.lo.toFixed(2)
        + ' and "' + rec.hiAt + '" at ' + rec.hi.toFixed(2),
      'the solver takes each line end elevation from its own centreline, so the static head round any loop through'
        + ' this junction is wrong by that much; draw the ends at one height, or make the node a volume if the'
        + ' lines are meant to enter a body');
  }
...
const copyRows = (rows) => {
  ...
  for (let i = 0; i < rows.length; i++) out[i] = [+rows[i][0], +rows[i][1]];
  return out;
};
...
function normShape(s) {
  ...
  if (s.kind === 'cylinder') {
    return { kind: 'cylinder', r: +s.r, len: +s.len, y0: num(s.y0, 0), axis: s.axis === 'z' ? 'z' : 'x' };
  }
  if (s.kind === 'annulus') return { kind: 'annulus', outer: copyRows(s.outer), inner: copyRows(s.inner) };
  const key = s.kind === 'lathe' ? 'profile' : 'table';
...
function normNode(raw, defaults, toK) {
  ...
  const fl = raw.fluid == null ? defaults.fluid : raw.fluid;
  // A NODE WHOSE FLUID IS STEAM IS STEAM, and its quality has to say so. The
  // field defaults to zero, which is water, and every state function keys off
  // it: hOf asks the LIQUID branch when the quality is zero, so a boundary
  // authored `fluid: "steam"` at 115 C and 1.1 bar was built as saturated
  // water at 467 kJ/kg, its temperature clamped down to the saturation line,
  // and the vent line leaving it carried a mixture at 120 kg/m3. Measured on
  // the consumer's containment vent: 140 kg/s at 2.3 m/s where twelve
  // kilogrammes of steam at 27 m/s is what half a bar over the atmosphere
  // through a K of 150 actually gives, and the drawn speed of that line comes
  // from the flow. Stating x explicitly still wins, so a wet steam node is
  // authored the same way it always was.
  const vapour = hasFluid(fl) && fluid(fl).vapour === true;
  const rec = {
    id: raw.id,
    kind: raw.kind,
    ...
    free,
    // A GAS SPACE IS OPEN OR IT IS CLOSED, and that is the whole difference
    // between a tank vented to the room and a pressuriser. Open, the pressure
    // is held at what the space is open TO, which is what `p` means and what
    // this library has always done. Closed, `p` is where the space STARTS and
    // its pressure is a state from then on: the steam in it, the water under
    // it and the volume they share decide it together. Same object, different
    // numbers, which is the only way a boiler shell, a condenser hotwell, a
    // pool under a lid and a tank open to the air can be one kind of thing.
    gas: { p: gasP, closed: isObj(raw.gas) ? raw.gas.closed === true : false },
    // A VOLUME THAT STATES A GAS PRESSURE MEANS IT, FREE OR NOT. This used to
    // fall back to one atmosphere for a volume that was not free, so a reactor
    // vessel authored water-solid at 155 bar with `gas: { p }` came up at 1 bar
    // with its 310 C water flashed to steam and 300 kg in a 480 cubic metre
    // vessel. A water-solid vessel is the normal state of a primary circuit,
    // so it is the first thing a migration meets.
    p: num(raw.p, gasP),
    T: toK(num(raw.T, toK === degK ? 15 : 288.15)),
    x: clamp(num(raw.x, vapour ? 1 : 0), 0, 1),
...
function normDevice(raw, defaults) {
  ...
  if (raw.kind === 'pump') {
    ...
    // Metres of head over saturation this pump needs at its suction before it
    // makes its full head. Left unstated it takes NPSH_REQ, which is a metre.
    if (raw.npsh != null) d.npsh = Math.max(0, num(raw.npsh, 0));
  } else if (raw.kind === 'valve') {
    ...
    // Both are live: a host drives them from its own model between steps, the
    // way it drives a pump's speed or a valve's command.
    d.dp = Math.max(0, num(raw.dp, 0));
    d.W = num(raw.W, 0);
  }
  return d;
}
