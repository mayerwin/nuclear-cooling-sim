...
const APP_DRAWN = /^(wall$|dome$|liner|slab$|floor$|fuel_rod_|sg_tube_|cond_tube_|coil$|sg_divider$|_cut_)/;
// Parts with a real wall, or solid through and through, whose cut face the
// unit draws solid through the stencil (see section.js): the vessels, the
// drums, the plates, the floors and walls of the tanks. Not the pipes (an
// open tube would count as solid right through) and not the open shades.
export const WALLED = /^(rpv_shell$|sg_shell$|sg_tubesheet$|turb_casing$|cond_shell$|cond_plate_|.*_casing$|.*_motor$|pool_floor$|pool_wall_|tank_floor$|tank_wall_|grav_valve$)/;
...
export function instantiate({ passive, cut }) {
  ...
  src.traverse((n) => {
    ...
    if (!n.isMesh) return;
    // the walled parts merge apart from the rest, so they can be mirrored
    // into the stencil without the open shapes that share their material
    const key = (WALLED.test(n.name) ? 'walled:' : '') + (n.material.name || n.material.uuid);
  ...
  for (const [key, bin] of bins) {
    ...
    mesh.name = key.startsWith('walled:') ? 'walled_' + key.slice(7) : 'steel_' + key;
    mesh.castShadow = !m.transparent;
    mesh.receiveShadow = true;
    group.add(mesh);
    byName[mesh.name] = mesh;
    if (key.startsWith('walled:')) walled.push(mesh);
  }
  ...
  for (const n of live) {
    ...
    byName[c.name] = c;
    if (c.isMesh && WALLED.test(c.name)) walled.push(c);
