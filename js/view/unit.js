...
import { pipe, vessel, tube, slab, V, roundedPath } from './parts.js?v=d7d0f53269';
import { liquidMaterial, steamMaterial, rippleNormal, Riser, Drip, Bubbles, frameOf, setGradient, gradientise, twoOctaveFlow, LOWFX, SEA_TILE } from './fluid.js?v=d7d0f53269';
import { tempColor, waterColor, heatOf, loopHeat, paleSRGB } from './materials.js?v=d7d0f53269';
import { Leg, Circuit, Surface, FLUID, clamp, lerp, hash1 } from '../flow.js?v=d7d0f53269';
import { Machines } from '../machines.js?v=d7d0f53269';
import { makeHydro, networkJson } from './hydro.js?v=d7d0f53269';
import { instantiate, getLayout } from './model.js?v=d7d0f53269';
import { SectionCap } from './section.js?v=d7d0f53269';
import { attach as attachFluids, FluidFrame } from '../../vendor/fluidsim/three/index.js?v=d7d0f53269';
import { build as buildFluidMats } from '../../vendor/fluidsim/three/materials.js?v=d7d0f53269';

// The water, the steam and every pipe's contents are drawn by the fluid
// library from the solver's own state: the model (tools/blender/plant.py)
// gives every vessel a wall and every pipe a bore through it, the network
// (tools/network.mjs) gives every line the same centreline run on into the
// water it serves, and the library draws what the solver says is there. The
// sim's own hand-built bodies stay for ?draw=sim (the picture before the
// library) and ?draw=lib (the library's volumes with the sim's pipes).
const DRAW_MODE = (() => { try { return new URLSearchParams(location.search).get('draw') || 'lib2'; } catch (e) { return 'lib2'; } })();
const DRAW_LIB = DRAW_MODE === 'lib' || DRAW_MODE === 'lib2';
// lib2: the pipes as the library's edge bodies as well, the tube banks and
// the coil included (the network states each bank as the library's bundle,
// nested U-tubes centred on the middle one)
const DRAW_LIB_EDGES = DRAW_MODE === 'lib2';
...
// the boiler shell's radius at a height above its base (the layout's profile)
const SG_SHELL = [[2.05, 2.7], [9.1, 2.7], [10.8, 3.1], [12.6, 4.0], [16.7, 4.0]];
function sgShellR(y) {
  for (let i = 1; i < SG_SHELL.length; i++) {
    const [y0, r0] = SG_SHELL[i - 1], [y1, r1] = SG_SHELL[i];
    if (y <= y1) return y <= y0 ? r0 : r0 + (r1 - r0) * (y - y0) / (y1 - y0);
  }
  return SG_SHELL[SG_SHELL.length - 1][1];
}
const _lvl = new THREE.Vector3();
// the reactor vessel's radius at a height above its base (the layout's
// profile, which the Blender model is built from)
function rpvR(y) {
  const prof = getLayout().rpv.profile;
...
export class Unit {
  constructor(plant, stage, worldX, worldZ = 0) {
    ...
    // The steel's cut face. Every walled part of the model (a vessel, a
    // drum, a plate, a floor, a tank wall) goes into a second stencil pass
    // after the concrete's and is capped with one cut-metal material, so a
    // wall reads as the band of metal it is and a nozzle bored through it
    // as the gap it is, with the pipe's casing showing through the gap. The
    // twins are children of their part, so they hide and move with it.
    this.steelSection = new SectionCap(this.cut[0], 3);
    for (const w of inst.walled) {
      for (const t of this.steelSection.mirror(w)) {
        t.position.set(0, 0, 0); t.quaternion.identity(); t.scale.set(1, 1, 1);
        w.add(t);
      }
    }
    {
      const cutSteel = new THREE.MeshStandardMaterial({ color: 0xb4bcc4, roughness: 0.5, metalness: 0.45 });
      inst.group.add(this.steelSection.cap(64, 44, new THREE.Vector3(0, 0, 1), new THREE.Vector3(7.5, 15, 0), cutSteel));
    }
  ...
  buildBuilding() {
    ...
    // Where a line passes through the wall it runs in a sleeve: a closed
    // cylinder of the pipe's own radius a hair SHORTER than the wall (past the
    // skins the count would be minus one, and the cap would paint the
    // overhang; the missing centimetre shows the casing, as the sleeve does), counted
    // INWARD, so that inside it the count comes back to zero and the cap
    // leaves the pipe's casing showing, cut open with its water in it. The
    // steam and feed lines on the turbine side, the vent on the other; each
    // found where its centreline crosses the wall's radius.
    {
      const Lp = getLayout().pipes;
      const sleeveMat = new THREE.MeshBasicMaterial();
      for (const spec of [Lp.steam, Lp.feed, Lp.vent]) {
        const pts = spec.pts;
        for (let i = 0; i + 1 < pts.length; i++) {
          const [x0, y0] = pts[i], [x1, y1] = pts[i + 1];
          for (const sx of [1, -1]) {
            const X = sx * (R_IN + WALL / 2);
            if (Math.min(x0, x1) - 1e-6 > X || Math.max(x0, x1) + 1e-6 < X || Math.abs(y1 - y0) > 1e-6) continue;
            const sleeve = new THREE.Mesh(
              new THREE.CylinderGeometry(spec.dia / 2, spec.dia / 2, WALL - 0.02, 24).rotateZ(Math.PI / 2), sleeveMat);
            sleeve.position.set(X, y0, 0);
            for (const t of this.section.mirror(sleeve, true)) g.add(t);
          }
        }
      }
    }
  ...
  buildVessels() {
    ...
    // The water FILLS the vessel, bottom head included: a lathe of the
    // vessel's own profile (a finger inside the wall), clipped at the level
    // by a world-space plane the frame loop moves. A cylinder scaled to the
    // level left the bottom head an empty bowl under a flat-bottomed column.
    const rpvProf = getLayout().rpv.profile.map(([r0, y0]) => new THREE.Vector2(Math.max(0, r0 - 0.08), y0));
    this.coreWater = new THREE.Mesh(new THREE.LatheGeometry(rpvProf, 48), m.water);
    this.coreWater.position.set(r.x, r.base, r.z);
    this.rpvLevelPlane = new THREE.Plane(new THREE.Vector3(0, -1, 0), 0);
    ...
    this.coreWater.material.clippingPlanes = [...this.mHalfRpv.clippingPlanes, this.rpvLevelPlane];
    // The gradient runs up the water, cold at the bottom to hot at the level:
    // along v on a lathe, and v is rewritten against the level each time it
    // moves (a lathe's own v goes by profile point, not by height).
    this.coreWater.material.userData.g.axis.value = 1;
    this.coreWater.material.normalMap.repeat.set(6, 4);
    this.coreWaterH = -1;
    ...
    // The downcomer: the ring of water between the column and the shell. The
    // shell is FULL of water up to the level, the column rising in the middle
    // and this ring falling round it, so it is drawn as the ring it is, to
    // the shell's own profile, clipped at the level. The feed line then ends
    // inside water, and nothing crosses an empty space. (Two thin strips
    // outside the column used to stand for it, with the feed's pipe or its
    // water crossing a metre and a half of nothing to reach them.)
    const ringProf = [[2.64, 3.0], [2.64, 9.1], [3.04, 10.8], [3.94, 12.6], [3.94, 16.7],
      [2.3, 16.7], [2.3, 3.0], [2.64, 3.0]].map(([r0, y0]) => new THREE.Vector2(r0, y0));
    this.sgDown = new THREE.Mesh(new THREE.LatheGeometry(ringProf, 40), liquidMaterial(0.9));
    this.sgDown.position.set(s.x, SG_BASE, s.z);
    this.sgLevelPlane = new THREE.Plane(new THREE.Vector3(0, -1, 0), 0);
    this.sgDown.material.clippingPlanes = [...this.cut, this.sgLevelPlane];
    ...
    g.add(this.sgDown);
    // its free surface: a ring at the level, as wide as the shell is there
    this.sgRingTop = new THREE.Mesh(new THREE.RingGeometry(2.35, 3.94, 40).rotateX(-Math.PI / 2), this.sgDown.material);
    this.sgRingTopR = 3.94;
    g.add(this.sgRingTop);
  ...
  buildLoop() {
    ...
    this.hot = pipe([V(r.x - 3.1, HOT_Y, r.z), V(s.x + HEAD_R_HOT - 0.5, HOT_Y, s.z)],
    ...
    this.coldB = pipe([
      V(p.x + 1.5, COLD_Y, p.z), V(r.x - 3.1, COLD_Y, r.z)
  ...
  buildSteamSide() {
    ...
    this.turbSteam = new THREE.Mesh(
      new THREE.CylinderGeometry(3.5, 2.3, X1 - X0 - 0.2, 32, 1, true).rotateZ(-Math.PI / 2),
      steamMaterial());
    ...
    this.exhaust = pipe([V(X1 - 1.3, AX - 3.225, t.z), V(X1 - 1.3, CTOP - 0.05, t.z)],
    ...
    // the steam pipe.
    this.steam = pipe([
      V(s.x, SG_BASE + 18.95, s.z), V(s.x, 31.4, s.z),
      V(SX, 31.4, t.z), V(SX, 14.0, t.z), V(X0 + 1.2, 14.0, t.z), V(X0 + 1.2, AX + 2.65, t.z)
  ...
  buildSafety() {
    ...
    if (this.passive) {
      ...
      const up = pipe([
        V(r.x + 3.1, HOT_Y, r.z), V(RUP, HOT_Y, r.z), V(RUP, COIL_Y, r.z)
      ...
      const dn = pipe([
        V(RDN, COIL_Y, r.z), V(RDN, COLD_Y + 1.4, r.z), V(r.x + 3.1, COLD_Y + 1.4, r.z)
      ], 0.45, m, { bend: 1.6, ...this.pipeOpts });
            dn.kindBreak = 'prhr';
...
// ---------------------------------------------------------------------------
import { ratedMdot, naturalMdot, THERMAL_W } from '../flow.js?v=d7d0f53269';
import { Plume, PuffCloud } from './plume.js?v=d7d0f53269';
...
Object.assign(Unit.prototype, {
  ...
  // The vessels' water as the library's bodies (?draw=lib). The network's
  // volumes are built whole and cut by the unit's own section planes; the
  // pipes stay the sim's for now (the network's edges are not built here),
  // and so does anything the network has no shape for (the condenser is an
  // area table, the pressuriser says draw: false). Levels, temperatures and
  // boiling come from the solver each frame through FluidFrame.
  buildLibFluids() {
    // this unit's own network (its design's lines only), as the solver has it
    const json = (this.hydro && this.hydro.json) || networkJson();
    if (!json) { this.fluids = null; return; }
    try {
      const netDraw = {
        nodes: (json.nodes || []).map((n) => Object.assign({}, n)),
        edges: DRAW_LIB_EDGES ? (json.edges || []) : [],
        devices: DRAW_LIB_EDGES ? (json.devices || []) : []
      };
      const mats = buildFluidMats({ cut: this.cut });
      const group = new THREE.Group();
      group.name = 'lib_fluids';
      this.root.add(group);
      this.fluids = attachFluids(netDraw, null, { root: group, cut: this.cut, casing: false, tracers: DRAW_LIB_EDGES, particles: false, vapour: false, mats });
      this.fluidFrame = new FluidFrame(this.fluids);
      console.log('lib fluids:', this.fluids.volumes.map((b) => b.id).join(' '));
    } catch (e) {
      console.warn('lib fluids: not built', e);
      this.fluids = null;
    }
  },
  ...
  update(st, dt, dtPlant = dt) {
    ...
    this.solve(st);
    this.solveTemps(st);
    // the library's solver, holding what was just decided (hydro.js)
    if (this.hydro === undefined) this.hydro = makeHydro(this);
    if (this.hydro) this.hydro.frame(dtPlant, st);
    if (DRAW_LIB && this.hydro && this.hydro.ok && this.fluids === undefined) this.buildLibFluids();
    ...
    _lvl.set(0, wy, 0);
    this.root.localToWorld(_lvl);
    this.rpvLevelPlane.constant = _lvl.y;
    {
      const h = Math.max(0.5, wy - L.rpv.base);
      if (Math.abs(h - this.coreWaterH) > 0.02) {
        const geo = this.coreWater.geometry, uv = geo.attributes.uv, pos = geo.attributes.position;
        for (let i = 0; i < uv.count; i++) uv.setY(i, Math.min(1, Math.max(0, pos.getY(i) / h)));
        uv.needsUpdate = true;
        this.coreWaterH = h;
      }
    }
    {
      const rr = Math.max(0.3, rpvR(wy - L.rpv.base) - 0.08);
      if (Math.abs(rr - this.coreTopR) > 0.04) {
        this.coreTop.geometry.dispose();
        this.coreTop.geometry = new THREE.CircleGeometry(rr, 48, 0, Math.PI * 2).rotateX(-Math.PI / 2);
        this.coreTopR = rr;
      }
    }
    ...
    ripple(this.coreTop, this.surfCore, this.coreTopR, 0.6);
    ...
    // boiler is actually asked to carry heat
    {
      ...
      // the downcomer ring: clipped at the level, feedwater-coloured (it is
      // what the feed lands in), its free surface as wide as the shell there
      {
        const lvl = SG_TS + sgH;
        _lvl.set(0, lvl, 0);
        this.root.localToWorld(_lvl);
        this.sgLevelPlane.constant = _lvl.y;
        const dm = this.sgDown.material;
        dm.normalMap.offset.y += dt * (carrying ? 1.4 : 0.35);
        paintFluid(dm, colourIn(this.rangeSec, this.legFeed.T1, cTmp));
        this.sgDown.visible = sgH > 0.3;
        const rr = sgShellR(lvl - SG_BASE) - 0.06;
        if (Math.abs(rr - this.sgRingTopR) > 0.04) {
          this.sgRingTop.geometry.dispose();
          this.sgRingTop.geometry = new THREE.RingGeometry(2.35, rr, 40).rotateX(-Math.PI / 2);
          this.sgRingTopR = rr;
        }
        this.sgRingTop.position.set(L.sg.x, lvl - 0.01, L.sg.z);
        this.sgRingTop.visible = this.sgDown.visible;
      }
    ...
    // the library's bodies in place of the sim's own (?draw=lib): painted,
    // levelled and rippled from the solver, after the sim has finished
    // deciding what its own bodies show, so its writes cannot undo this
    if (this.fluids) {
      for (const m of [this.coreWater, this.coreTop, this.sgWater, this.sgDown, this.sgRingTop, this.poolWater, this.tankWater, this.headHot, this.headCold, this.condWater, this.condTop]) if (m) m.visible = false;
      if (DRAW_LIB_EDGES) {
        for (const q of this.pipes) {
          if (q.core) q.core.visible = false;
          for (const cap of q.caps || []) cap.visible = false;
          if (q.bub && q.bub.mesh) q.bub.mesh.visible = false;
        }
        for (const u of [...(this.sgTubes || []), ...(this.condTubes || [])]) if (u.mesh) u.mesh.visible = false;
        if (this.poolCoil && this.poolCoil.mesh) this.poolCoil.mesh.visible = false;
      }
      if (this.fluidFrame && this.hydro && this.hydro.solver) this.fluidFrame.update(dt, this.hydro.solver);
    }
  }
});
...
// for hydro.js: the vessel's water level as an absolute height, from the
// plant's fraction (after the class, or it is read before it exists)
Unit.prototype.waterLevelY = function (lvl) { return waterY(lvl); };
