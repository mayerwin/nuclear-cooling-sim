...
export class SectionCap {
  ...
  // order: the render order of the stencil pass; the cap draws at order + 1.
  // Two solids capped in different materials (the concrete, the steel) take
  // consecutive pairs, each cap clearing the count its own pass left.
  constructor(plane, order = 1) {
    this.plane = plane;
    this.order = order;
    const base = {
